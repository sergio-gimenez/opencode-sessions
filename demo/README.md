...
`Ctrl+T` to cycle the Claude target, `Enter` or `Tab` to "open" — which prints
the command it *would* have run instead of running it.
...
Running the demo is then just `HOME=<that>` — no path overrides, and nothing
...
Both scripts unset `CLAUDE_CONFIG_DIR`, `CLAUDE_PROJECTS_PATH`,
`OPENCODE_DB_PATH` and `OCS_CONFIG_PATH` so nothing from the recorder's own
environment leaks into the recording.
